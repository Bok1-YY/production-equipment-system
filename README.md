...
| **维修闭环** | 待接单、已接单、到场核对报修信息、维修、结构化试运行、结单全流程，记录响应时间、诊断原因、维修方法、零件与返修关系；待试运行只显示试运行选项，维修资料有误需显式返回维修；巡检转入的工单结单前强制拍摄维修完成照片。 |
| **巡检与计划任务** | 支持设备巡检、结构化点检、一/二/三级保养模板、周期计划、到期任务和异常转维修，保留问题发现与维修完成两阶段现场照片。 |
...
随后双击仓库根目录或桌面的 `一键启动全功能设备系统.bat`。启动器固定连接 `equipment-system/factory-data/equipment.db`；该文件缺失时会拒绝创建空库并明确报错。启动器自动检测当前私网 IPv4；APK 缺失、移动端源码更新或 Wi-Fi 地址变化时，会先自动准备 Android 构建环境并重新打包。确认 APK 可下载后，启动器再启动完整后端、打开电脑页面并显示手机安装地址。首次构建需要确认 Android SDK 许可并下载依赖，可能耗时数分钟，以后会复用本机缓存。服务在后台常驻，关闭启动窗口不会中断；需要停止时双击 `equipment-system\stop-windows.bat`。启动失败时，错误会显示在窗口中，服务日志保存在 `equipment-system\data\server-*.log`。电脑和手机共用同一套账号、功能与数据，不再区分多个“一键启动”类别。
...
已到场 ── 核对故障设备与故障分类；两项确认后才能开工
   ↓
维修中 ── 填诊断原因、维修方法、零件和必要的完成照片；错误零件可删除重填
   ↓
待试运行 ── 只选择正常 / 带问题可运行 / 无法运行；资料有误显式返回维修
           结单检查缺项可点击，自动返回并定位到对应表单
   ↓
...
- 巡检转成维修工单后，技工修完还必须通过现场相机拍摄至少一张“维修完成照片”，否则服务端拒绝结单；
...
服务器已经运行旧版 Docker、需要从 GitHub 构建最新版并迁移本地数据库时，先按[Docker 服务器安全升级说明](./Docker服务器升级说明.md)做只读摸底。新版容器在数据库挂载缺失时会拒绝启动，源码构建可叠加 `compose.build.yaml`，一致性备份默认永不自动删除。
...
| `PORT` | `8787` | HTTP 服务端口 |
| `YSM_DB_PATH` | `data/equipment.db` | 手动启动的 SQLite 路径；Windows 一键启动显式覆盖为 `factory-data/equipment.db` |
...
│   ├── deploy/                 # Caddy 生产配置
│   ├── demo-data/              # 工厂源库快照；视为现场数据，不直接运行
│   ├── factory-data/           # Windows 日常运行库与附件（不入 Git）
│   ├── docs/                   # 设备编码与组合规则
...
当前 151 项测试覆盖设备编码、组合变动、身份权限、照片魔数、扫码解析、维修状态机、Docker 缺库拒启、待试运行禁止越级修改维修资料、错误零件删除、核对报修信息、结构化试运行、评价照片可见性、巡检问题照片与维修完成照片双重强制、运营报表下钻、点检保养、Excel 导入、登录锁定、幂等和安全响应边界。Android 相关改动还必须在真实手机上完成安装、冷启动、登录、首页数据加载以及对应硬件流程和 instrumentation 验证，不能用模拟器或“APK 构建成功”替代。
